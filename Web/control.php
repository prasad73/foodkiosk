...
If (isset($_GET['action'])){
	...
	print '
		...
		<font face="verdana">
		<p align="center">Vendigo Access console</p>
		';
		...
		$query3 = mysqli_query($dbConnection, "SELECT UserID, password, salt FROM users WHERE username = '$username';");

				$UID1 = mysqli_fetch_assoc($query3);
				$salt = $UID1['salt'];
				$pswd = $UID1['password'];
		...
		print '<tr><th>Box #</th><th>Order</th><th>Name</th><th>Phone</th><th>Status</th><th>Action</th><th>Edit</th></tr>';
		while ($currentBox < $totalBoxes){
			...
			print '<tr>';
			print '<td align="center">' . $Box . '</td><td>' . $Orderno . '</td><td align="center"></td><td align="center"></td><td align="center"><img src="' . $image . '" width="50"></td><td align="center" valign="middle"><form name="pin' . $Box . 'edit" action="index.php" method="get"><input type="hidden" name="action" value="' . $action . '"><input type="hidden" name="pin" value="' . $Box . '"><input type="submit" value="' . $buttonValue . '"></form></td><td><form name="pin' . $Box . '" action="index.php" method="get"><input type="hidden" name="action" value="edit"><input type="hidden" name="pin" value="' . $Box . '"><input type="submit" value="Edit"></form></td>';
			print '</tr>';
			$currentBox ++;
		}
		print '</table>';
